const SHEET_ID = '1e1SfQZTkgOJ44hTPoclI8UTKqHavF2qtXW6XtZCWxpQ';
...
const SENKINO_GID = 0;
const KUVEKINO_GID = 1783652729; // <-- change to your real gid if needed
...
function buildCsvUrls({ gid, sheetName }) {
  ...
  // Сначала gviz: отдаёт CSV с docs.google.com без редиректа на googleusercontent,
  // где /export/… часто даёт 400 в консоли, хотя следующий URL потом «спасает» загрузку.
  ...
  // Без gid/sheet не добавляем «голый» export — лишние запросы и неверный лист при нескольких вкладках.
  if (typeof gid !== 'number' && !sheetName) {
...
async function initApp() {
  try {
    ...
    // сортировка, лист 2 (desktop + mobile)
    setupSortableHeaderRow(document.querySelector('#lotsKuvekino > div > div'), 'kuvekino', onSort);
    setupSortableHeaderRow(document.querySelector('#lotsKuvekinoMobile .listLots > div'), 'kuvekino', onSort);
